...
//				.andExpect(jsonPath("$[0].hypeLevel",  is(50)))
				...
				.andExpect(jsonPath("$[0].versions", hasSize(2))).andExpect(jsonPath("$[0].versions[0]", is("0.9RC-1")))
				...
				.andExpect(jsonPath("$[1].versions", hasSize(1))).andExpect(jsonPath("$[1].id", is(2)))
		...
		mockMvc.perform(
				...
				.andDo(MockMvcResultHandlers.print()).andExpect(status().isCreated()).andExpect(jsonPath("$.id", is(3)))
				.andExpect(jsonPath("$.name", is("JQuery")))
				.andExpect(jsonPath("$.hypeLevel", is(EHypeLevel.LOW.name())))
				.andExpect(jsonPath("$.versions", hasSize(3))).andExpect(jsonPath("$.versions[0]", is("1.0")))
				.andExpect(jsonPath("$.versions[1]", is("2.0"))).andExpect(jsonPath("$.versions[2]", is("3.0")))
		...
		mockMvc.perform(get("/frameworks/3").contentType(MediaType.APPLICATION_JSON)
				.content(mapper.writeValueAsBytes(framework))).andDo(MockMvcResultHandlers.print())
		...
		mockMvc.perform(get("/search/JQuery").contentType(MediaType.APPLICATION_JSON)
				.content(mapper.writeValueAsBytes(framework))).andDo(MockMvcResultHandlers.print())
				.andExpect(status().isOk()).andExpect(jsonPath("$", hasSize(1))).andExpect(jsonPath("$[0].id", is(3)))
				.andExpect(jsonPath("$[0].name", is("JQuery")))
				.andExpect(jsonPath("$[0].hypeLevel", is(EHypeLevel.LOW.name())))
				.andExpect(jsonPath("$[0].versions", hasSize(3))).andExpect(jsonPath("$[0].versions[0]", is("1.0")))
				.andExpect(jsonPath("$[0].versions[1]", is("2.0"))).andExpect(jsonPath("$[0].versions[2]", is("3.0")))
	...
	public void addFrameworkInvalid() throws JsonProcessingException, Exception {
		...
		mockMvc.perform(
				...
				.andExpect(jsonPath("$.errors", hasSize(1))).andExpect(jsonPath("$.errors[0].field", is("name")))
