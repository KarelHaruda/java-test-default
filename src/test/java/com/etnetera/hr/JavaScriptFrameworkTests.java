...
//		JavaScriptFramework react = new JavaScriptFramework("ReactJS", cal.getTime(), 50);
	...
	public void frameworksTest() throws Exception {
		...
		mockMvc.perform(get("/frameworks")).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8))
				...
				//proto se zde porovnává na EHypeLevel.XXXX.name() což je vlastně to co je v té JSON odpovědi, kterou zde testujeme uloženo
				//pravděpodobně by se nechal vytořit správný Enum ze stringu z použitím Enum.valueOf(EHypeLevel.class, text_z_Json), 
				//ale pro účely testování by porovnání těch dvou stringů mělo být plně dostačující
...
//				.andExpect(jsonPath("$[0].hypeLevel",  is(50)))
	...
	public void addFrameworkInvalid() throws JsonProcessingException, Exception {
		...
		mockMvc.perform(post("/add").contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsBytes(framework)))
				...
				//což způsoboivalo že tento unit test celý skončil chybou už zde na kontrole status().isBadRequest()
				//Bylo nutné doimplementovat obsluhu REST endpointu /add a v dále zařídit validaci objektu před uložením a
				//případné validační chyby předat jako objekt do odpovědi aby se to zde správně otestovalo
